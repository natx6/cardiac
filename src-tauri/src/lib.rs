...
/// One row of a customers/patients export from the old system. `phone` and
/// `opening_balance` are optional — an export may only carry names, or only
/// names + what each customer already owes.
#[derive(Deserialize)]
...
fn complete_sale_impl(
    ...
    // Per-transaction discounts are capped by the manager-set ceiling
    // (settings.max_discount_pct, default 100 = no cap). Anyone at the
    // counter may grant up to the ceiling; the server re-checks so a forged
    // payload can never exceed it.
    let ceiling: f64 = tx
        .query_row(
            "SELECT COALESCE((SELECT CAST(value AS REAL) FROM settings WHERE key = 'max_discount_pct'), 100)",
            [],
            |r| r.get(0),
        )
        .map_err(|e| e.to_string())?;
    if disc > ceiling + 1e-9 {
        return Err(format!(
            "Discount {:.0}% exceeds the allowed maximum of {:.0}%",
            disc, ceiling
        ));
    }
...
fn return_sale_impl(
    ...
    // Refunds are open to every role (two-tap confirm + audit trail on the
    // frontend); only voids stay manager-gated.
...
fn commit_customer_import(
    ...
    for (i, r) in records.iter().enumerate() {
        ...
        match existing {
                ...
                tx.execute(
                    "UPDATE patients
                     SET phone = CASE WHEN (?2 IS NOT NULL AND (phone IS NULL OR phone = '')) THEN ?2 ELSE phone END,
                         opening_balance = MAX(COALESCE(opening_balance, 0), ?3)
                     WHERE id = ?1",
                    rusqlite::params![id, phone, opening],
                )
                .map_err(|e| e.to_string())?;
                updated += 1;
            }
            None => {
                tx.execute(
                    "INSERT INTO patients (name, phone, opening_balance, created_at)
                     VALUES (?1, ?2, ?3, datetime('now', 'localtime'))",
                    rusqlite::params![name, phone, opening],
                )
                .map_err(|e| e.to_string())?;
                created += 1;
            }
...
fn commit_supplier_import(
    ...
    for (i, r) in records.iter().enumerate() {
        ...
        let email = r.email.as_deref().map(str::trim).filter(|s| !s.is_empty());
        let opening = r.opening_balance.unwrap_or(0.0);
        let opening = if opening < 0.0 { 0.0 } else { opening };
        ...
        match existing {
                ...
                tx.execute(
                    "UPDATE suppliers
                     SET phone = CASE WHEN (?2 IS NOT NULL AND (phone IS NULL OR phone = '')) THEN ?2 ELSE phone END,
                         ...
                         email = CASE WHEN (?5 IS NOT NULL AND (email IS NULL OR email = '')) THEN ?5 ELSE email END,
                         opening_balance = MAX(COALESCE(opening_balance, 0), ?4)
                     WHERE id = ?1",
                    rusqlite::params![id, phone, location, opening, email],
                )
                .map_err(|e| e.to_string())?;
                updated += 1;
            }
            None => {
                tx.execute(
                    "INSERT INTO suppliers (name, phone, location, email, opening_balance, created_at)
                     VALUES (?1, ?2, ?3, ?4, ?5, datetime('now', 'localtime'))",
                    rusqlite::params![name, phone, location, email, opening],
                )
                .map_err(|e| e.to_string())?;
                created += 1;
            }
...
/// Bulk-replace the FDA catalog in one transaction (shared by
/// `refresh_fda_catalog`). Plain helper, not a Tauri command — there is no
/// `import_fda_catalog` IPC route.
fn import_fda_catalog_impl(
    app: &AppHandle,
    drugs: Vec<FdaDrug>,
    operator: Option<String>,
    role: Option<String>,
...
async fn refresh_fda_catalog(
    ...
    import_fda_catalog_impl(&app, all, operator, role)
}
...
fn create_user(
    ...
    if role != "owner" && role != "manager" && role != "mca" {
        return Err("Role must be owner, manager or mca".into());
    }
    let conn = open_db(&app)?;
...
fn update_user(
    ...
    if let Some(r) = role {
        ...
        if r != "owner" && r != "manager" && r != "mca" {
            return Err("Role must be owner, manager or mca".into());
        }
        // Prevent demoting the last owner/manager.
        if r == "mca" {
            let mgr_count: i64 = conn
                .query_row("SELECT COUNT(*) FROM users WHERE role IN ('owner', 'manager') AND is_active = 1 AND id != ?1", [id], |row| row.get(0))
                .map_err(|e| e.to_string())?;
            let this_role: String = conn
                .query_row("SELECT role FROM users WHERE id = ?1", [id], |row| row.get(0))
                .map_err(|_| "User not found".to_string())?;
            if (this_role == "owner" || this_role == "manager") && mgr_count == 0 {
                return Err("Cannot demote the last active owner/manager".into());
            }
        }
    ...
    if let Some(active) = isActive {
        // Prevent deactivating the last owner/manager.
        if !active {
            let mgr_count: i64 = conn
                .query_row("SELECT COUNT(*) FROM users WHERE role IN ('owner', 'manager') AND is_active = 1 AND id != ?1", [id], |row| row.get(0))
                .map_err(|e| e.to_string())?;
            let this_role: String = conn
                ...
                .map_err(|_| "User not found".to_string())?;
            if (this_role == "owner" || this_role == "manager") && mgr_count == 0 {
                return Err("Cannot deactivate the last active owner/manager".into());
            }
        }
...
fn create_product(
    ...
    let tx = conn
        ...
        .map_err(|e| e.to_string())?;
    tx.execute(
        "INSERT INTO products (name, barcode, category, supplier, strength, generic_name, active_ingredient, unit, dosage_form, route, cost_price, selling_price, stock_qty, reorder_level, pack_size, batch_no, expiry_date)
        ...
        rusqlite::params![
            ...
            product.unit.as_deref().map(|s| s.trim()).filter(|s| !s.is_empty()),
            product.dosage_form.as_deref().map(|s| s.trim()).filter(|s| !s.is_empty()),
            product.route.as_deref().map(|s| s.trim()).filter(|s| !s.is_empty()),
            product.cost_price,
...
mod tests {
    ...
    #[test]
    fn complete_sale_enforces_discount_ceiling() {
        let mut conn = test_db();
        let pid = insert_product(&conn, "Aspirin", 5.0, 10.0, 100);
        conn.execute(
            "INSERT INTO settings (key, value) VALUES ('max_discount_pct', '20')",
            [],
        )
        .unwrap();
        // 10% sails through (total 18.0 on 2 × 10.0).
        complete_sale_impl(
            &mut conn,
            vec![cash(18.0)],
            vec![line(pid, "Aspirin", 2, 10.0)],
            None,
            None,
            None,
            None,
        ...
        .expect("discount within the ceiling must succeed");
        // 25% must be refused even though the tendered cash covers it.
        let result = complete_sale_impl(
            &mut conn,
            vec![cash(20.0)],
            vec![line(pid, "Aspirin", 2, 10.0)],
        ...
        assert!(result.is_err(), "discount above the ceiling must be rejected");
    }
    ...
    fn returns_open_to_all_roles() {
        ...
        // Refunds are open to every role: no PIN, wrong PIN, and correct PIN
        // must all succeed (voids stay manager-gated — see below).
        ...
        .expect("return without a PIN must succeed");
        assert!((ok.total_refunded - 15.0).abs() < 0.001);
        let ok2 = return_sale_impl(
            &mut conn,
            sale.sale_id,
            None,
            None,
        ...
        .expect("return with any PIN value must succeed");
        assert!((ok2.total_refunded - 15.0).abs() < 0.001);
        let ok3 = return_sale_impl(
            &mut conn,
            sale.sale_id,
            None,
            None,
        ...
        assert!((ok3.total_refunded - 15.0).abs() < 0.001);
