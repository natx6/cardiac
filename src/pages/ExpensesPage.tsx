...
export function ExpensesPage() {
  ...
  /** MCA accounts record expenses but never see the books (no list,
   * totals, breakdown, or delete) — viewing stays owner/manager-only. */
  ...
  const load = async () => {
    // MCA add-only: never fetch the books for counter accounts.
    if (useStore.getState().currentUser?.role === "mca") {
      setExpenses([]);
      setSummary(null);
      return;
    }
  ...
  return (
    <div className="flex h-full flex-col overflow-auto bg-surface-container-lowest p-margin-page">
      ...
      {isMca && (
        <p className="mb-4 max-w-xl rounded border border-outline-variant bg-surface px-3 py-2 text-body-sm text-on-surface-variant">
          Logged expenses go straight to the books — only managers and the owner can view or delete them.
        </p>
      )}
