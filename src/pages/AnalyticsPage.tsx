...
export function AnalyticsPage() {
  ...
  /** MCA accounts see Reports scoped to their own sales only: the operator
   * filter locks to their display name and business-wide sections
   * (supplier balances, controlled register, audit trail, expense books)
   * stay hidden. */
  const isMca = currentUser?.role === "mca";
  const selfName = currentUser?.display_name ?? "";
  ...
  const effOpFilter = isMca ? selfName : opFilter;
  const cashOp = effOpFilter === "All" ? null : effOpFilter;
  useEffect(() => {
    void (async () => {
      try {
        const db = await initDb();
  ...
  // Owner accounts are excluded from the roster (visually separate from
  // shift staff) — their sales still count in the totals, just not as a
  // selectable cashier. Never fetched for MCA accounts.
  ...
  useEffect(() => {
    ...
    listUsers()
      .then((us) => {
        const owners = us.filter((u) => u.role === "owner").map((u) => u.display_name);
        setOwnerNames(owners);
        if (owners.includes(opFilter)) setOpFilter("All");
      })
      .catch(() => setOwnerNames([]));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isMca]);
  const opOptions = useMemo(() => {
    ...
    for (const on of ownerNames) names.delete(on);
  ...
  }, [operators, report, ownerNames]);  const load = useCallback(async () => {
    const effOp = isMca ? selfName || "__none__" : opFilter === "All" ? null : opFilter;
    try {
      const [rep, ctrl] = await Promise.all([
        ...
        // Business-wide ledgers never load for MCA accounts.
        isMca
    ...
    if (isMca) {
      setSuppliers([]);
      setExpenses({ total: 0, byCategory: [] });
      setAudit([]);
      return;
    }
    supplierBalances()
    ...
    loadAuditLog(from, to)
      .then(setAudit)
      .catch(() => setAudit([]));
  }, [from, to, opFilter, methodFilter, isMca, selfName]);
  ...
  // MCA accounts never see the expense books — drop those tiles.
  const kpis = [
    ...
    <div className="flex h-full flex-col overflow-auto bg-surface-container-lowest p-margin-page">
      <div className="mb-4 flex items-end justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {isMca ? (
            <span
              title="MCA accounts see only their own sales"
              className="flex h-8 items-center rounded border border-outline-variant bg-surface-container-low px-2 text-label-md font-label-md text-on-surface-variant"
            >
              Only mine
            </span>
          ) : (
          ...
          {report?.recent.map((r) => (
            ...
            >
              ...
              <span className="flex shrink-0 items-center gap-2">
                {/* Voids erase a sale entirely — owner/manager only. Refunds
                    (Return) are open to every role. */}
                {!isMca && report.voidableId === r.id && (
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
