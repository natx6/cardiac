...
import { useStore, isPrivileged } from "../store/useStore";
import {
  backupDbToDir,
  getSettings,
  isManagerPinSet,
...
export function SettingsPage() {
  ...
  const maxDiscountPct = useStore((s) => s.maxDiscountPct);
  const [maxDisc, setMaxDisc] = useState(String(maxDiscountPct));
  ...
  /** Users — owner/manager add logins; MCA accounts are counter-only. */
  ...
  const save = async () => {
    ...
    try {
      ...
      // Per-sale discount ceiling (manager-set): anyone at the counter may
      // grant up to this %; the server rejects anything above it.
  ...
  // MCA accounts get Settings for the theme only — everything else
  // (pharmacy, users, PIN, backups, FDA) stays owner/manager-only.
      ...
      <div className="flex h-full flex-col overflow-auto bg-surface-container-lowest p-margin-page">
        <div className="mb-6">
          ...
          <p className="text-body-sm font-body-sm text-on-surface-variant">
            Personal appearance only — the rest of Settings needs a manager or the owner.
          </p>
        </div>
        {settingsErr && (
        ...
        <div className="max-w-xl">{appearanceCard}</div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full flex-col overflow-auto bg-surface-container-lowest p-margin-page">
      ...
      <div className="max-w-xl">
        ...
        <label className="mb-4 block">
          <span className="mb-1 block text-body-md font-body-md text-on-surface">
            Max sale discount (%) — the most anyone may grant on one transaction (0 = no discounts)
          </span>
          <input
            value={maxDisc}
        ...
        {isManager && (
          <div className="mb-6 rounded-xl border border-outline-variant bg-surface p-4">
            <h3 className="mb-3 text-headline-md font-headline-md text-on-surface">Logins — owner/manager adds accounts</h3>
            <p className="mb-3 text-body-sm text-on-surface-variant">
              Owner and manager see everything · MCA works the counter (own reports, theme-only settings, add-only expenses). The last active owner/manager can't be demoted or deactivated.
            </p>
                ...
                <div key={u.id} className="flex items-center justify-between gap-2 px-3 py-2">
                  <div className="min-w-0">
                    <p className="truncate text-body-sm font-semibold text-on-surface">
                      ...
                      <span className={`ml-2 rounded-full px-1.5 py-0.5 text-[10px] font-bold ${u.role === "owner" ? "bg-amber-400/20 text-amber-700 dark:text-amber-200" : u.role === "manager" ? "bg-primary/10 text-primary" : "bg-surface-variant text-on-surface-variant"}`}>{u.role}</span>
