...
export function RestockPage() {
  ...
  const doReceive = async () => {
    ...
    try {
      ...
      // Optional on-the-spot payment, recorded right after the stock lands —
      // one step instead of receive-then-pay. Overpay is rejected server-side;
      // clamp here too so a stale total can't overshoot.
      let payMsg = "";
      if (payAmt > 0) {
        const balance = Math.max(0, detail.p.total_amount - detail.p.paid_amount);
        const amt = Math.min(payAmt, balance);
        if (amt <= 0) {
          payMsg = " (nothing left to pay)";
        } else {
          try {
            const pr = await recordPayment(
              detail.p.id,
              amt,
              payMethod,
              currentUser?.display_name ?? null,
            ...
            payMsg =
              pr.balance > 0.005
                ? ` · ${fmtMoney(pr.paid)} paid, ${fmtMoney(pr.balance)} left`
                : " · fully paid";
          } catch (pe) {
      ...
      const done = r.complete
        ? `${r.reference_no} received — ${r.added} unit${r.added === 1 ? "" : "s"} added to stock${payMsg}.`
        : `${r.reference_no}: received ${r.added} — the rest is still outstanding${payMsg}.`;
            ...
            <div className="flex items-center justify-between border-b border-outline-variant bg-surface-container-low px-6 py-4">
              <div>
                ...
                <p className="text-body-sm font-body-sm text-on-surface-variant">
                  ...
                  {[detail.p.supplier_phone, detail.p.supplier_email].filter(Boolean).length > 0
                    ? ` · ${[detail.p.supplier_phone, detail.p.supplier_email].filter(Boolean).join(" · ")}`
                    : ""}
