...
export function PosPage() {
  ...
  // Per-transaction discount, set by hand at the counter and clamped to the
  // manager-set ceiling from Settings (enforced again server-side).
  const setDiscountClamped = (v: number) => {
    const n = Number.isFinite(v) ? v : 0;
    setDiscountPct(Math.min(Math.max(0, n), maxDiscountPct));
  };
      ...
      <section className="flex w-96 flex-shrink-0 flex-col overflow-hidden rounded-xl border border-outline-variant bg-surface shadow-sm">
        ...
        <div className="flex flex-col gap-3 border-t border-outline-variant bg-surface-container-low p-4">
          <div className="mb-2 space-y-1">
            ...
            {maxDiscountPct > 0 && (
              <div className="flex items-center justify-between font-data-mono text-body-sm text-on-surface-variant">
                <label htmlFor="pos-discount">Discount % (max {maxDiscountPct})</label>
                <input
                  id="pos-discount"
                  type="number"
                  min={0}
                  ...
                  step="any"
                  value={discountPct || ""}
                  onChange={(e) => setDiscountClamped(Number(e.target.value))}
                  placeholder="0"
