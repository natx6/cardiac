...
import { useStore, roleLabel } from "../store/useStore";
...
export function Sidebar() {
  ...
  // MCAs see everything except Requisitions (ordering/receiving/paying).
  // Reports and Settings render scoped for them (own data / theme only).
  const visibleNav = isMca ? NAV.filter((n) => n.id !== "restock") : NAV;
        ...
        <div className="relative mt-4 px-3">
                ...
                <span
                  className={`shrink-0 rounded px-1 text-[9px] font-bold uppercase tracking-wider ${
                    currentUser?.role === "owner"
                      ...
                      : role === "manager"
                        ? "bg-primary/25 text-primary-fixed"
                        : "bg-white/10 text-white/60"
                  }`}
                >
                  {currentUser ? roleLabel(currentUser.role) : role}
                </span>
              </span>
          ...
          {!currentUser && pickerOpen && (
            ...
            >
              ...
              {role === "manager" ? "Lock — back to counter" : "Unlock Manager mode"}
            </button>
          )}
        </div>
      </div>
