...
const PAY_TERMS = ["Cash", "Credit"] as const;
...
export function PurchaseModal({
  ...
  // New requisitions start as Draft; legacy "30/60 Days" terms on old orders
  // are preserved untouched (display-only) by keeping editP's stored value.
  const [status, setStatus] = useState<string>(editP?.status ?? "Draft");
            ...
            <div className="col-span-7 grid grid-cols-4 gap-3">
              ...
              <label className="block">
                ...
                <select value={payTerm} onChange={(e) => setPayTerm(e.target.value)} className={inputCls}>
                  ...
                  {/* Legacy terms on old orders (e.g. "30 Days") stay visible
                      but can't be chosen for new ones. */}
                  {!PAY_TERMS.includes(payTerm as (typeof PAY_TERMS)[number]) && (
                    <option value={payTerm}>
                      {payTerm} (legacy)
                    </option>
                  )}
                </select>
