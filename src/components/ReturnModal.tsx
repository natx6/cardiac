...
export function ReturnModal({ sale, onClose, onDone }: Props) {
  ...
  const doReturn = async () => {
    ...
    // Refunds are open to every role (voids stay manager-only) — no PIN.
    setBusy(true);
    setErr("");
    try {
      ...
      beep(true);
      setSlip(r);
      // Refresh the global product cache BEFORE showing the slip: callers
      // fire their own (unawaited) refreshes in onDone, and on slower disks
      // the user could otherwise navigate back to the POS grid while the
      // cache was still stale. Awaiting here closes that race.
      try {
        await useStore.getState().refreshProducts();
      } catch {
        // A failed refresh must not hide a successful refund — callers retry.
      }
      onDone();
    } catch (e) {
      setErr(String(e).replace(/^Error: /, ""));
