...
export async function getSettings(): Promise<AppSettings> {
  ...
  return {
    ...
    maxDiscountPct: Number(map.max_discount_pct ?? 100) || 0,
 ...
 * location / email / opening balance (take the larger owed); no match → create. */
...
export type UserRole = "owner" | "manager" | "mca";
...
export async function updateUser(
  id: number,
  opts: { display_name?: string; role?: UserRole; is_active?: boolean },
  actor?: string | null,
  actorRole?: string | null,
): Promise<AppUser> {
...
export async function loadSuppliers(): Promise<Supplier[]> {
  const d = await initDb();
  return await d.select<Supplier[]>("SELECT id, name, phone, location, email FROM suppliers ORDER BY name");
}
...
export async function addSupplier(
  ...
  const rows = await d.select<{ id: number }[]>(
    "INSERT INTO suppliers (name, phone, location, email) VALUES ($1, $2, $3, $4) " +
      "ON CONFLICT(name) DO UPDATE SET phone = COALESCE($2, suppliers.phone), location = COALESCE($3, suppliers.location), email = COALESCE($4, suppliers.email) " +
      "RETURNING id",
    [name.trim(), phone?.trim() || null, location?.trim() || null, email?.trim() || null],
  );
  if (!rows[0]?.id) {
...
export async function loadPurchases(): Promise<Purchase[]> {
  ...
  return await d.select<Purchase[]>(
    `SELECT pu.id, pu.reference_no, pu.supplier_id, pu.supplier_name, pu.purchase_date,
            ...
            s.phone AS supplier_phone, s.location AS supplier_location, s.email AS supplier_email,
...
// ---- Per-transaction discounts ----
// Discounts are set by hand on each sale at the counter (clamped to the
// manager-set max_discount_pct ceiling, enforced again in complete_sale).
// There is intentionally no per-customer standing discount: the old
// patients.discount_tier column is left dormant in the schema.
