...
- Top bar: pharmacy name, search field (Ctrl+K focuses it; matches products
  ...
  Requisitions.
- Color-coded theme (Material blue `#0060a8` primary, red errors), Inter
...
- **Product grid**: cards show name, strength, unit of measure (blister/
  ...
  status (blue = healthy, yellow = at/below reorder level, red = out of
...
- Status logic: blue = stock above reorder level; yellow = at/below reorder
...
- **Daily cash-up**: pick a day, enter the opening float (remembered per day),
  ...
  what was counted, and save the variance (blue/red) to a per-day history.
